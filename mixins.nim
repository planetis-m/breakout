import gametypes, heaparray, vmath, utils, registry, storage, fusion/smartptrs
...
proc mixCollide*(world: var World, entity: Entity, size = vec2(0, 0)) =
   mixBody HasCollide
   world.collide[entity.index] = Collide(size: size,
...
proc mixDraw2d*(world: var World, entity: Entity, width, height = 100'i32,
   ...
   mixBody HasDraw2d
   world.draw2d[entity.index] = Draw2d(width: width, height: height, color: color)

proc mixFade*(world: var World, entity: Entity, step = 0.0) =
   mixBody HasFade
   world.fade[entity.index] = Fade(step: step)
...
proc mixHierarchy*(world: var World, entity: Entity, parent = invalidId) =
   ...
   world.hierarchy[entity.index] = Hierarchy(head: invalidId, prev: invalidId,
...
proc mixMove*(world: var World, entity: Entity, direction = vec2(0, 0), speed = 10.0) =
   mixBody HasMove
   world.move[entity.index] = Move(direction: direction, speed: speed)

proc mixPrevious*(world: var World, entity: Entity, position = point2(0, 0),
      rotation = 0.Rad, scale = vec2(1, 1)) =
   mixBody HasPrevious
   world.previous[entity.index] = Previous(position: position,
...
proc mixTransform2d*(world: var World, entity: Entity, trworld = mat2d(), translation = vec2(0, 0),
   ...
   world.transform[entity.index] = Transform2D(world: trworld, translation: translation,
         rotation: rotation, scale: scale)
